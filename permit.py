...
permit_type = [ 'ID', 'Type', 'BuildingPermit_id']
building_permit = ['ID', 'PermitNumber', 'Status']
...
for row in dreader:
    buildingpermit_list.append({'ID': i, 'PermitNumber': row['Application/Permit Number'], 'Status': row['Status']})
    i += 1
...
#new table
table_05 = open('/Users/alisonjkolberg/Code/303/table_05.csv', 'w')

my_dwriter = csv.DictWriter(table_05, fieldnames = permit_type)
my_dwriter.writeheader()
a = open('/Users/alisonjkolberg/Code/303/building_permits.csv')
dreader = csv.DictReader(a)
#loop_dreader = csv.DictReader(f)
...
for row in dreader:
    if row ['Permit Type'] not in dup_list:
        dup_list.append(row['Permit Type'])
...
#for row in loop_dreader:
for row in dup_list:
    permittype_list.append({'ID': i, 'Type': row,'BuildingPermit_id':''})
    i += 1



my_dwriter.writerows(permittype_list)
table_05.close()
a.close()
